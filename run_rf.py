...
def create_table(seq,top_kmer_word,top_kmer_hits):
    table = [top_kmer_word]*len(top_kmer_hits)
    
    cont = True
    while cont:
        cont = expand_forward(seq,top_kmer_hits,table)
...
if __name__ == "__main__":
    ...
    for key in seqs:
        ...
        kmer_dict = index_seq(seqs[key])
        top_kmer_word,top_kmer_hits = find_top_kmer(kmer_dict)
        table = create_table(seqs[key],top_kmer_word,top_kmer_hits)
